...
export default async function addReaction(message: Message, emoji: string): Promise<void> {
    ...
    if ((permissions) && ((permissions & 0x40) !== 0x40)) throw new Error("Missing permissions to add reactions");
    if ((permissions) && (/[A-Za-z0-9_:]/.test(emoji)) && ((permissions & 0x40000) !== 0x40000)) throw new Error("Missing permissions to use external emojis");
