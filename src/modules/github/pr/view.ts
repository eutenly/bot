...
export default function view(data: GitHubPR | undefined, message: Message, metadata: any) {
    ...
    // Pull requests
    else if (["pullrequests", "prs"].includes(input.toLowerCase().replace(/\s+/g, ""))) return prs(message, metadata.ownerName, metadata.name);
