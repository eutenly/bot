...
export default function embed(command: Command, data: ListedIssue[]): Embed {
    ...
    const embed = new Embed()
        .setAuthor("GitHub Issues", "https://i.imgur.com/FwnDNtd.png")
    ...
    if (data.length === 0) return embed
        .setDescription("There aren't that many issues")
        .setColor(0xf44242);
    ...
    data.forEach((d: ListedIssue, i: number) => embed.addField(null, `**${i + 1}. [${d.title}](https://github.com/${command.metadata?.ownerName}/${command.metadata?.name}/issues/${d.number})**\n${truncateString(d.text.replace(/[\n\r]/g, " "), 80)}`));
