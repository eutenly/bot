...
export default function embed(command: Command, data: ListedRelease[]): Embed {
    ...
    const embed = new Embed()
        .setAuthor("GitHub Releases", "https://i.imgur.com/FwnDNtd.png")
    ...
    if (data.length === 0) return embed
        .setDescription("There aren't that many releases")
        .setColor(0xf44242);

    // Build embed
    embed
        .setAuthor(`${command.metadata?.ownerName}/${command.metadata?.name}: Releases`, "https://i.imgur.com/FwnDNtd.png", `https://github.com/${command.metadata?.ownerName}/${command.metadata?.name}/releases`)
