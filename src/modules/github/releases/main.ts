...
export default async function main(userRequest: UserRequest, ownerName: string, name: string, commandHistoryIndex?: number): Promise<Command | undefined> {
    ...
    const command: Command = new Command(userRequest.client, {
        ...
        getData: (input: string = "", page: number = 1): string => `https://api.github.com/repos/${encodeURIComponent(ownerName)}/${encodeURIComponent(name)}/releases?per_page=5${page ? `&page=${page}` : ""}`,
...
export function url(ownerName: string, name: string): ViewDataURL {
    ...
    return `https://github.com/${ownerName}/${name}/releases`;
}