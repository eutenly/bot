...
export default function view(data: GitHubIssue | undefined, message: Message, command: Command) {
    ...
    // Issues
    else if (input.toLowerCase().replace(/\s+/g, "") === "issues") return issues(message, command.metadata.ownerName, command.metadata.name);
