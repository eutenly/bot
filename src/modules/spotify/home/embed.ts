...
export default function embed(command: Command, data?: Home): Embed {
    ...
    const embed = new Embed()
        ...
        .setColor(0x1ed760)
        .addField("Your Playlists", `${playlists.map((p: ListedPlaylist, i: number) => `**p-${i + 1}. [${p.name}](https://open.spotify.com/playlists/${p.id})** - ${p.tracks.toLocaleString()} Track${p.tracks === 1 ? "" : "s"}`).join("\n")}\n\n\u2022 Use \`${prefix}view <Playlist Number>\` to view a playlist\n\u2022 Use \`${prefix}view playlists\` to view more of your playlists\n\u200b`)
        .addField("Your Top Tracks", `${topTracks.map((t: ListedTrack, i: number) => `**t-${i + 1}. [${t.name}](https://open.spotify.com/track/${t.id})** by [${t.artist.name}](https://open.spotify.com/artist/${t.artist.id}) - ${parseDuration(t.length)}`).join("\n")}\n\n\u2022 Use \`${prefix}view <Track Number>\` to view a track\n\u2022 Use \`${prefix}spotify top tracks\` to view more of your top tracks\n\u200b`)
        .addField("Your Top Artists", `${topArtists.map((a: ListedArtist, i: number) => `**a-${i + 1}. [${a.name}](https://open.spotify.com/artist/${a.id})** - ${a.followers.toLocaleString()} Follower${a.followers === 1 ? "" : "s"}`).join("\n")}\n\n\u2022 Use \`${prefix}view <Artist Number>\` to view an artist\n\u2022 Use \`${prefix}spotify top artists\` to view more of your top artists\n\u200b`)
        .addField("Recently Played", `${recentlyPlayed.map((t: ListedTrack, i: number) => `**r-${i + 1}. [${t.name}](https://open.spotify.com/track/${t.id})** by [${t.artist.name}](https://open.spotify.com/artist/${t.artist.id}) - ${parseDuration(t.length)}`).join("\n")}\n\n\u2022 Use \`${prefix}view <Track Number>\` to view a track\n\u2022 Use \`${prefix}spotify history\` to view more of your recently played tracks`)
        .setBranding();
