...
export default function embed(command: Command, data: SpotifySearchResult[]): Embed {
     ...
     */
    if (command.metadata?.type === "track") data.forEach((d: SpotifySearchResult, i: number) => embed.addField(null, `**${i + 1}. [${d.name}](https://open.spotify.com/track/${d.id})**\n[${d.artist?.name}](https://open.spotify.com/artist/${d.artist?.id}) - ${parseDuration(d.length || 0)}`));
     ...
     * 910,840 Followers
     */
    else if (command.metadata?.type === "artist") data.forEach((d: SpotifySearchResult, i: number) => embed.addField(null, `**${i + 1}. [${d.name}](https://open.spotify.com/artist/${d.id})**\n${d.followers?.toLocaleString()} Follower${d.followers === 1 ? "" : "s"}`));
     ...
     */
    else if (command.metadata?.type === "album") data.forEach((d: SpotifySearchResult, i: number) => embed.addField(null, `**${i + 1}. [${d.name}](https://open.spotify.com/album/${d.id})**\n[${d.artist?.name}](https://open.spotify.com/artist/${d.artist?.id}) - ${d.tracks?.toLocaleString()} Track${d.tracks === 1 ? "" : "s"}`));
