...
export default function embed(command: Command, data?: TwitterHome): Embed {
    ...
    const embed = new Embed()
        ...
        .setColor(0x1da1f2)
        .addField("Your Tweets", `${data.tweets.map((t: TwitterTweet, i: number) => `**t-${i + 1}. [${t.user.name} (@${t.user.handle})](https://twitter.com/${t.user.handle}/status/${t.id})**: ${t.text.replace(/\n/g, " ").substring(0, 50)}${t.text.replace(/\n/g, " ").length > 50 ? "..." : ""}`).join("\n")}\n\n\u2022 Use \`${prefix}view <Tweet Number>\` to view a Tweet\n\u2022 Use \`${prefix}view tweets\` to view more of your Tweets\n\u200b`)
        .addField("Your Timeline", `${data.timeline.map((t: TwitterTweet, i: number) => `**tl-${i + 1}. [${t.user.name} (@${t.user.handle})](https://twitter.com/${t.user.handle}/status/${t.id})**: ${t.text.replace(/\n/g, " ").substring(0, 50)}${t.text.replace(/\n/g, " ").length > 50 ? "..." : ""}`).join("\n")}\n\n\u2022 Use \`${prefix}view <Tweet Number>\` to view a Tweet\n\u2022 Use \`${prefix}twitter timeline\` to view your timeline\n\u200b`)
        .setBranding();
