...
type Stat = UserInitiatedGuildEvent | WebsiteInitiatedUserEvent | GuildInitiatedEvent | ResourceUsageStat | CacheStat;
...
type CacheType = "channels" | "messages" | "users";
...
export default async function collectStat(client: Client, stat: Stat) {
    ...
    else if (stat.type === "cacheStat") await client.postgres.query("INSERT INTO cache_stats VALUES(NOW(), $1, $2)", [stat.cacheType, stat.amount]);
}